...
ALTER TABLE STUDENT ADD CONSTRAINT STUDENT_SUBJECT_SUB_CODE_FK FOREIGN KEY(SUB_CODE) REFERENCES SUBJECT(SUB_CODE) ON DELETE CASCADE;
...
INSERT INTO SUBJECT VALUES (00011,'생명과학');
--제약조건 설정 확인
SELECT *
FROM USER_CONSTRAINTS
...
SELECT * FROM EMPLOYEES;
--EMPLOYEES 와 DEPARTMENTS 의 INNER JOIN
SELECT * 
...
SELECT EMPLOYEE_ID, FIRST_NAME, JOB_ID, SALARY, E.DEPARTMENT_ID, DEPARTMENT_NAME,
        CASE
        WHEN UPPER(DEPARTMENT_NAME) = UPPER('Marketing') THEN SALARY * 1.05
        WHEN UPPER(DEPARTMENT_NAME) = UPPER('Purchasing') THEN SALARY * 1.10
        WHEN UPPER(DEPARTMENT_NAME) = UPPER('Human Resources') THEN SALARY * 1.15
         WHEN UPPER(DEPARTMENT_NAME) = UPPER('IT') THEN SALARY * 1.20
         END NEW_SALARY
